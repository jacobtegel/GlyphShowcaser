...
if font is not None and font.path:
	fontPath = os.path.dirname(font.path) #Get Folder in which the font is
...
if exportAs == 0:
	...
	if glyphSelection == 0:
		 saveImage(f"{outputFolder}/{time}-LayerOverlayer-{fontName}-{glyph.name}.pdf")

	else:
		 saveImage(f"{outputFolder}/{time}-LayerOverlayer-{fontName}.pdf")
	
if exportAs == 1:

	if glyphSelection == 0:
		saveImage(f"{outputFolder}/{time}-LayerOverlayer-{fontName}-{glyph.name}.svg")

	else:
		saveImage(f"{outputFolder}/{time}-LayerOverlayer-{fontName}.svg")
   
if exportAs == 2:
	
	if glyphSelection == 0:
		saveImage(f"{outputFolder}/{time}-LayerOverlayer-{fontName}-{glyph.name}.png")
	
	else:
		saveImage(f"{outputFolder}/{time}-LayerOverlayer-{fontName}.png")
